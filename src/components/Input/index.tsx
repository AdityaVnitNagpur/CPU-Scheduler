...
const Input = (props: InputProps) => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (burstTimeArr.includes(0)) {
      ...
      alert('Number of the arrival times and burst times are not equal'
      );
      return;
    }

    else if (arrivalTime.trim() === '' || burstTime.trim() === '') {
      alert('Please enter arrival time or burst time values');
      return;
    }

    else if (arrivalTimeArr.some(isNaN) || burstTimeArr.some(isNaN)) {
      alert('Arrival times or burst times entered incorrectly');
      return;
    }
    
    else if (selectedAlgo.value === 'RR' && isNaN(timeQuantumInt)) 
